...
def searchNanoDrug_2(NN_ADJ,ADJ_sign,nano,drug,chemical,disease,queryInput,elemName):
...
def searchNanoDisease_2(NN_ADJ,ADJ_sign,nano,drug,chemical,disease,queryInput,elemName):
	print ' In Search Clique 2'
	#Nano - Drug - Disease
	cliques_NDrDi = set()
	#Nano - Drug - Disase -Chemical
...
def searchNanoChemical_2(NN_ADJ,ADJ_sign,nano,drug,chemical,disease,queryInput,elemName):
...
def searchDrugDisease_2(NN_ADJ,ADJ_sign,nano,drug,chemical,disease,queryInput,elemName):
...
def searchDrugChemical_2(NN_ADJ,ADJ_sign,nano,drug,chemical,disease,queryInput,elemName):
	print ' In searchDrugDisease_2'
	#Nano - Drug - Disease
	cliques_NDrDi = set()
	#Nano - Drug - Disase -Chemical
...
def searchDiseaseChemical_2(NN_ADJ,ADJ_sign,nano,drug,chemical,disease,queryInput,elemName):
	print ' In searchDrugDisease_2'
	#Nano - Drug - Disease
	cliques_NDrDi = set()
	#Nano - Drug - Disase -Chemical
...
'''
#Implementation by intersection
def searchNanoDrug(NN_ADJ,ADJ_sign,nano,drug,chemical,disease,queryInput,elemName):
	#Nano - Drug - Disease
	cliques_NDrDi = set()
	#Nano - Drug- Chemical
	cliques_NDrC = set()
	...
	for nn in range(len(nanoInput)): #for each nano
		nanoIndex = nanoInput[nn]
		for dd in range(len(drugInput)): #for each drug
			drugIndex = drugInput[dd]
			nano_drug = NN_ADJ[nanoInput[nn],drugInput[dd]]!=0 #nano-drug connection (True/False)
			if(nano_drug):
				disease_nano = NN_ADJ[nanoIndex,:] * disease != 0
				disease_drug = NN_ADJ[drugIndex,:] * disease != 0
				disease_adj = disease_nano & disease_drug
				if(np.sum(disease_adj)>0):
					disease_adj = [d for d in np.where(disease_adj == True)[0]]
					for di in  range(len(disease_adj)):
						diseaseIndex = disease_adj[di]
				...
				chemical_nano = NN_ADJ[nanoIndex,:] * chemical != 0
				chemical_drug = NN_ADJ[drugIndex,:] * chemical != 0
				chemical_adj = chemical_nano & chemical_drug
				if(np.sum(chemical_adj)>0):
					chemical_adj = [d for d in np.where(chemical_adj == True)[0]]
					for cc in  range(len(chemical_adj)):
						chemicalIndex = chemical_adj[cc]
	...
	for i in range(len(cliques_NDrDi)):
		NDrDi = list(cliques_NDrDi)[i][0]
		for j in range(len(cliques_NDrC)):
			NDrC = list(cliques_NDrC)[j][0]
			if NN_ADJ[NDrDi[2],NDrC[2]]!=0:

				cliques_index = (NDrDi[0],NDrDi[1],NDrDi[2],NDrC[2])
				cliques_names = (elemName[NDrDi[0]],elemName[NDrDi[1]],elemName[NDrDi[2]],elemName[NDrC[2]])
				cliques_edges = (ADJ_sign[NDrDi[0],NDrDi[1]],ADJ_sign[NDrDi[0],NDrDi[2]],ADJ_sign[NDrDi[0],NDrC[2]],ADJ_sign[NDrDi[1],NDrDi[2]],ADJ_sign[NDrDi[1],NDrC[2]],ADJ_sign[NDrDi[2],NDrC[2]])
				#Clique Edges are in the order: nano-drug,nano-disease,nano-chem,drug-disea,drug-chem,disea-chem
				cliques_NDrDi.add((cliques_index,cliques_names,cliques_edges))

				cliques_index = (NDrC[0],NDrC[1],NDrDi[2],NDrC[2])
				cliques_names = (elemName[NDrC[0]],elemName[NDrC[1]],elemName[NDrDi[2]],elemName[NDrC[2]])
				cliques_edges = (ADJ_sign[NDrC[0],NDrC[1]],ADJ_sign[NDrC[0],NDrDi[2]],ADJ_sign[NDrC[0],NDrC[2]],ADJ_sign[NDrC[1],NDrDi[2]],ADJ_sign[NDrC[1],NDrC[2]],ADJ_sign[NDrDi[2],NDrC[2]])
				#Clique Edges are in the order: nano-drug,nano-disease,nano-chem,drug-disea,drug-chem,disea-chem
				cliques_NDrDi.add((cliques_index,cliques_names,cliques_edges))


	Cliques = {'NanoDrugDiseaseChemical':cliques_NDrDiC,'NanoDrugDisease':cliques_NDrDi,'NanoDrugChemical':cliques_NDrC}
	return Cliques

'''

'''
from networkStatistics import NDrDiC_Net
from networkStatistics import NDrDi_Net
from networkStatistics import NDrC_Net
from networkStatistics import NDiC_Net
from networkStatistics import DrDiC_Net
...
def searchNanoDrug_2(NN_ADJ,ADJ_sign,nano,drug,chemical,disease,queryInput,elemName):
				...
				if(np.sum(disease_adj)>0):
					...
					for di in  range(len(disease_adj)):
						...
						if(nano_drug & nano_disease & drug_disease):
							...
							Res = NDrDi_Net(nanoIndex,drugIndex,diseaseIndex,ADJ_sign,eIdx,elemName)
							eIdx = Res['eIdx']
							...
							if(np.sum(chemical_adj)>0):
								...
								for cc in range(len(chemical_adj)):
									...
									if(isClique):
										...
										Res = NDrDiC_Net(nanoIndex,drugIndex,diseaseIndex,chemicalIndex,ADJ_sign,eIdx,elemName)
										eIdx = Res['eIdx']
										nodes.update(Res['nodes'])
										edges.update(Res['edges'])
										...
										if(nano_drug & nano_chem & drug_chemical):
											...
											Res = NDrC_Net(nanoIndex,drugIndex,chemicalIndex,ADJ_sign,eIdx,elemName)
											eIdx = Res['eIdx']
											nodes.update(Res['nodes'])
											edges.update(Res['edges'])
										...
										if(nano_disease & nano_chem & disease_chemical):
											...
											Res = NDiC_Net(nanoIndex,diseaseIndex,chemicalIndex,ADJ_sign,eIdx,elemName)
											eIdx = Res['eIdx']
											nodes.update(Res['nodes'])
											edges.update(Res['edges'])
										...
										if(drug_disease & drug_chemical & disease_chemical):
											...
											Res = DrDiC_Net(drugIndex,diseaseIndex,chemicalIndex,ADJ_sign,eIdx,elemName)
											eIdx = Res['eIdx']
											nodes.update(Res['nodes'])
											edges.update(Res['edges'])
	...
	Cliques = {'NanoDrugDiseaseChemical':cliques_NDrDiC,'NanoDrugDisease':cliques_NDrDi,'NanoDrugChemical':cliques_NDrC,'NanoDiseaseChemical':cliques_NDiC,'DrugDiseaseChemical':cliques_DrDiC}
	NS = {'nodes':nodes, 'edges': edges}
	return {'Cliques':Cliques, 'NS':NS}
